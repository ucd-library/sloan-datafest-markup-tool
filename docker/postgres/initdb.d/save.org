...
#+RESULTS:
|  10822 | entry_name_with_conf.csv    |
| 231405 | entry_name_without_conf.csv |
| 242227 | total                       |
...
#+RESULTS:
: 137300 name_summary_global_stats.csv
...
#+RESULTS:
| name     | type_new | price_new |   price |
|----------+----------+-----------+---------|
| 9.99[.,] | price*   |     0.98. |    0.98 |
| 9.99[.,] | price*   |  1595.60. | 1595.60 |
| 9.99[.,] | price*   |     7.39. |    7.39 |
| 9.99[.,] | price*   |    95.24. |   95.24 |
...
** Count Summary
#+BEGIN_SRC sql
select 'country' as type,count(*) from (select distinct country from eng_jane.entries) as c
union
select 'province' as type,count(*) from (select distinct province from eng_jane.entries) as c
union
select 'region' as type,count(*) from (select distinct region from eng_jane.entries) as c
union
select 'variety' as type,count(*) from (select distinct country from eng_jane.entries) as c
order by 2 asc;
#+END_SRC
...
#+BEGIN_SRC sql
\COPY (select * from eng_jane.yearly_whisker) to eng_jane_yearly_whisker.csv with csv header
#+END_SRC
